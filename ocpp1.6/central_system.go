...
func (c *chargePointConnection) ID() string { return c.channel.GetID() }
func (c *chargePointConnection) TLSConnectionState() *tls.ConnectionState {
	return c.channel.GetTLSConnectionState()
}
...
func (cs *centralSystem) SetNewChargePointHandler(handler ChargePointConnectionHandler) {
	cs.server.SetNewClientHandler(func(chargePoint ws.Channel) {
		handler(&chargePointConnection{chargePoint})
	})
}
...
func (cs *centralSystem) SetChargePointDisconnectedHandler(handler ChargePointConnectionHandler) {
	cs.server.SetDisconnectedClientHandler(func(chargePoint ws.Channel) {
		for cb, ok := cs.callbackQueue.Dequeue(chargePoint.GetID()); ok; cb, ok = cs.callbackQueue.Dequeue(chargePoint.GetID()) {
